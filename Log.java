	...
	public static void debug(String title, String mesg) {
		...
		try
		{
			if (Log.DO_LOGGING)
			{
				...
				raf.writeUTF("<TR>" + "<TD>" + new Date() + "</TD>" + "<TD>" + title + "</TD>" + "<TD>" + mesg + "</TD></TR>");
			}
		}
	...
	public static void error(String title, String mesg) {
		...
		try
		{
			if (Log.DO_LOGGING)
			{
				...
				raf.writeUTF("<TR>" + "<TD style=\"color:#FF0000\">" + new Date() + "</TD>" + "<TD style=\"color:#FF0000\">" + title + "</TD>"
	...
	public static void error(String title, Exception mException) {
		...
		try
		{
			if (Log.DO_LOGGING)
			{
				...
				str = "<TR>" + "<TD style=\"color:#FF0000\">" + new Date() + "</TD>" + "<TD style=\"color:#FF0000\">" + title
						+ "</TD><TD style=\"color:#FF0000\"><BR/>" + mException.toString() + "<BR/><BR/>" + mException.getMessage() + "<BR/><BR/>";

				for (int ele = 0; ele < mStackTraceElement.length; ele++)
				{
					str += mStackTraceElement[ele].getClassName() + "." + mStackTraceElement[ele].getMethodName() + " ("
							+ mStackTraceElement[ele].getFileName() + " : " + mStackTraceElement[ele].getLineNumber() + ") <BR/>";
				}
